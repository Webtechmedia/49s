...
class Tank_auth
{
	 ...
	 * @return	array
	 */
	function create_user($first_name,$last_name,$role,$username, $email, $password, $email_activation)
	{
	...
	{
		if (strlen($user_id) > 0)   {
			return $this->ci->users->activate_user($user_id);
		}
		return FALSE;
	}
	function deactivate_user($user_id)
	{
		...
		if (strlen($user_id) > 0) {
			return $this->ci->users->deactivate_user($user_id);
		}
		return FALSE;
	}
	...
	{
		if ( $this->ci->users->delete_user($user_id)) {			// success
			return TRUE;
		} else {															// fail
			return FALSE;
		}
