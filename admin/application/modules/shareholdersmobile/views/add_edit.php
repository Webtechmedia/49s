<div class="container">
	<div class="row">
		<div class="col-xs-12">
			<h1 class="page-header">Shareholders mobile</h1>
		</div>
	</div>
	<div class="row">
		<div class="col-xs-12">
...
<script>
	$(document).ready(function() {
		...
		$( "#images_update" ).submit(function( event ) {
			event.preventDefault();
			//alert( "Handler for .submit() called." );
			var jqxhr = $.post( "<?php echo base_url('/shareholdersmobile/save_image_video' ) ?>", {
				url: $( "#url" ).val(),
				id: $( "#id" ).val(),
				...
				.done(function(data) {
					if(data.server_obj.success == true){
						...
						window.location = '<?php echo(base_url('shareholdersmobile')) ?>';
					} else {
						$("#error_header_uploader").hide();
	...
	function postImage(){
		$.ajax({
			url: "<?php echo base_url('/shareholdersmobile/prepare_image')  ?>",
			type: "POST",
