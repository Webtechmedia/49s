...
class Uploadsmobile extends CI_Controller
{
    ...
    function remove_content($id){
    	if(!$this->tank_auth->is_logged_in() || $this->tank_auth->is_role('admin')!=1){
            ...
            $this->db->where('id',$id);
        $this->db->delete('t_uploads_mobile');
        redirect(base_url('uploadsmobile'));
    }
    ...
    function save_image_video()
    {
        ...
        $this->load->model('JsonOutput');
        $this->load->model('Uploads_model');
                if($this->session->userdata('image_upload_temp') || $this->session->userdata('video_upload_temp') || $this->input->post('id',0) > 0 ){
